<template>
	<article
		...
		<div class="px-4">
			<button type="button" class="text-left text-[14.5px] font-semibold leading-snug text-stone-900 hover:text-accent-500"
				@click="drawer.open('book', book.bookID)">{{ book.title }}</button>
			<p class="mt-0.5 truncate text-[12.5px] text-stone-400">{{ authorLine(book) }}</p>
			<div class="mt-2">
		...
		<div class="mt-auto flex items-center justify-between gap-2 py-3.5 pl-4 pr-3">
			<ButtonsButton v-if="isInCart(book.bookID)" variant="ghost" size="sm" class="!border-emerald-200 !bg-emerald-50 !text-emerald-700"
				...
				<Icon name="i-lucide-check" class="h-3.5 w-3.5" />In cart
			</ButtonsButton>
			<ButtonsButton v-else variant="ghost" size="sm" class="!border-accent-100 !bg-accent-100 !text-accent-600"
				...
				<Icon name="i-lucide-plus" class="h-3.5 w-3.5" />Add to cart
			</ButtonsButton>
