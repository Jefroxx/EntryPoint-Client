<template>
	<div class="relative">
		<div v-if="modelValue"
			...
			<div class="min-w-0">
				<p class="truncate text-[13px] font-semibold text-stone-900">{{ getLabel(modelValue) }}</p>
				<p v-if="getSublabel" class="truncate text-[11px] text-stone-500">{{ getSublabel(modelValue) }}</p>
			</div>
		...
		<template v-else>
			<div
				...
				<input :id="inputId" v-model="query" type="text" :placeholder="placeholder" autocomplete="off"
					class="w-full border-none bg-transparent text-[13.5px] text-stone-900 outline-none placeholder:text-stone-400"
					@focus="openList" @blur="isOpen = false" />
			</div>

			<div v-if="isOpen"
				class="absolute left-0 right-0 top-[calc(100%+4px)] z-20 max-h-56 overflow-y-auto rounded-xl border border-stone-200 bg-white p-1 shadow-lg">
				<p v-if="loading" class="px-3 py-2 text-[12.5px] text-stone-400">Searching…</p>
				<p v-else-if="results.length === 0" class="px-3 py-2 text-[12.5px] text-stone-400">No matches.</p>
				<button v-for="(item, index) in results" :key="index" type="button"
					...
					<span class="text-[13px] font-semibold text-stone-900">{{ getLabel(item) }}</span>
					<span v-if="getSublabel" class="text-[11.5px] text-stone-500">{{ getSublabel(item) }}</span>
				</button>
			</div>
		</template>
	</div>
