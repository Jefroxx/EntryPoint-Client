<template>
	<div class="relative inline-flex gap-0.5 rounded-xl bg-stone-100 p-1">
		...
		<button v-for="tab in tabs" :key="tab.value" ref="btnRefs" type="button"
			class="relative z-10 inline-flex items-center gap-1.5 rounded-lg px-4 py-2 text-sm font-semibold transition-colors duration-200"
			:class="tab.value === modelValue ? 'text-stone-900' : 'text-stone-500 hover:text-stone-700'"
			@click="select(tab.value)">
			{{ tab.label }}
			<span v-if="tab.badge"
				class="inline-flex h-[17px] min-w-[17px] items-center justify-center rounded-full bg-amber-600 px-1.5 text-[10px] font-bold text-white">
				{{ tab.badge }}
			</span>
		</button>
	</div>
