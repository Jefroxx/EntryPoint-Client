	...
	<div class="flex h-full flex-col rounded-2xl border border-stone-200 bg-white p-5 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md">
		...
		<div class="flex-1 overflow-x-auto">
			<table class="w-full min-w-max text-left text-sm">
				<thead>
					<tr class="border-b border-stone-100 text-xs font-medium text-stone-500">
				...
				<tbody>
					<tr v-for="loan in loans" :key="`${loan.studentName}-${loan.bookTitle}`"
						...
						<td class="py-3">
							<span class="inline-flex items-center gap-1 rounded-full border border-red-200 bg-red-50 px-2.5 py-1 text-xs font-bold text-red-700">
					...
					<tr v-if="loans.length === 0">
						<td colspan="3" class="py-6 text-center text-sm text-stone-400">No overdue books.</td>
					</tr>
				</tbody>
			</table>
		</div>
