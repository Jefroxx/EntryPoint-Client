<template>
	<LibrarianTableShell :columns="['Student', 'Item', 'Qty', 'Points spent', 'Redeemed', 'Status', '']" :loading="loading"
		...
		<tr v-for="(redemption, index) in redemptions" :key="redemption.redemptionID"
			...
			<td class="px-4 py-3 text-[13.5px] font-medium text-stone-800">{{ redemption.item?.name ?? 'Removed item' }}</td>
			<td class="font-data px-4 py-3 text-[12.5px] text-stone-500">{{ redemption.quantity }}</td>
			<td class="font-data px-4 py-3 text-[12.5px] text-stone-500">{{ redemption.pointsSpent }} pts</td>
			<td class="font-data px-4 py-3 text-[12.5px] text-stone-500">{{ formatDate(redemption.redeemedAt) }}</td>
			<td class="px-4 py-3">
